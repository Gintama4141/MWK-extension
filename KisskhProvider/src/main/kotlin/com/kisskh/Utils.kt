...
suspend fun fetchtmdb(title: String?, year: Int?, isMovie: Boolean): Int? {
    ...
    fun matches(item: JSONObject, ignoreYear: Boolean): Boolean {
        ...
        val titleMatches = resultTitle.equals(title, ignoreCase = true) || resultTitle.contains(title, ignoreCase = true) || title.contains(resultTitle, ignoreCase = true)
        val yearMatches = ignoreYear || year == null || resultYear == null || year == resultYear
        return titleMatches && yearMatches
    }

    for (i in 0 until results.length()) {
        val item = results.optJSONObject(i) ?: continue
        if (matches(item, ignoreYear = false)) return item.optInt("id")
    }
    for (i in 0 until results.length()) {
        val item = results.optJSONObject(i) ?: continue
        if (matches(item, ignoreYear = true)) return item.optInt("id")
    }
    return null
}
