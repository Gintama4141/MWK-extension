...
suspend fun searchTorrentSources(query: String): List<TorrentSearchResult> {
    val seen = mutableSetOf<String>()
    val results = mutableListOf<TorrentSearchResult>()
    val searchQuery = query.replace(' ', '+')

    runCatching {
        val doc = app.get("https://uindex.org/search.php?search=$searchQuery&c=1", timeout = 10_000L).document
        doc.select("tr").forEach { row ->
            val title = row.select("td:nth-child(2) > a:nth-child(2)").text()
                .ifBlank { row.select("td:nth-child(2) a").lastOrNull()?.text().orEmpty() }
            val magnet = row.select("td:nth-child(2) > a:nth-child(1)").attr("href")
                .ifBlank { row.select("td:nth-child(2) a[href^=magnet:]").attr("href") }
            if (title.isNotBlank() && magnet.startsWith("magnet:") && magnet !in seen) {
                seen.add(magnet)
                results.add(TorrentSearchResult(title.trim(), magnet))
            }
        }
    }

    runCatching {
        for (cat in listOf("3000000", "2000000")) {
            val doc = app.get("https://knaben.org/search/$searchQuery/$cat/1/seeders", timeout = 10_000L).document
            doc.select("tr.text-nowrap.border-start, tr[class*=border-start]").forEach { row ->
