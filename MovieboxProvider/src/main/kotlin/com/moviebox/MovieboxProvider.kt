...
class MovieboxProvider : MainAPI() {
    ...
    override suspend fun getMainPage(
        ...
        if(!request.data.contains(",")) {
            ...
            val index = app.get(url).text.let { tryParseJson<Media>(it) }?.data?.subjectList?.map {
            ...
            val index = app.post("$mainAPIUrl/wefeed-h5api-bff/subject/filter", requestBody = body)
                .text.let { tryParseJson<Media>(it) }?.data?.items?.map {
                    it.toSearchResponse(this)
                } ?: throw ErrorLoadingException("No Data Found")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        ).text.let { tryParseJson<Media>(it) }?.data?.items?.map { it.toSearchResponse(this) }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val document = app.get("$secondAPIUrl/wefeed-h5-bff/web/subject/detail?subjectId=$id")
            .text.let { tryParseJson<MediaDetail>(it) }?.data
    ...
    override suspend fun loadLinks(
        ...
        val media = tryParseJson<LoadData>(data) ?: return false
        ...
        val streams = app.get(
            ...
            referer = referer
        ).text.let { tryParseJson<Media>(it) }?.data?.streams
        ...
        app.get(
            ...
            referer = referer
        ).text.let { tryParseJson<Media>(it) }?.data?.captions?.map { subtitle ->
