...
open class SeaTV : Donghuastream() {
    ...
    override suspend fun loadLinks(
        ...
        document.select(".mobius option").amap { server ->
            ...
            val doc = Jsoup.parse(base64Decode(base64))
            ...
            val url = iframeUrl.takeIf { it.isNotEmpty() } ?: metaUrl
            if (url.isEmpty()) return@amap
            when {
                url.contains("vidmoly") -> {
                    val newUrl = "http:" + url.substringAfter("=\"").substringBefore("\"")
                    loadExtractor(newUrl, referer = url, subtitleCallback, callback)
                }
                url.endsWith("mp4") -> {
                    callback.invoke(newExtractorLink("All Sub Player", "All Sub Player", url = url, INFER_TYPE) {
                        this.referer = ""
                        this.quality = getQualityFromName("")
                    })
                }
                else -> loadExtractor(url, referer = url, subtitleCallback, callback)
            }
        }
        return true
    }
